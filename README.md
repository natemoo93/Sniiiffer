...
**Where network calls live:** every manifest fetch runs in the background worker, never in `popup.js`. The popup asks via `sniiiffer:contentState` / `sniiiffer:highlightConfig` / `sniiiffer:resolveAsync`. Keeping fetches in one place means host permissions, timeouts and error handling apply uniformly; a popup also dies on blur, which would abort an in-flight request.

**Sync warning:** `common/detect.js` duplicates detection logic that also lives in `../prod/index.html` (and, for the canvas/manifest helpers, `../prod/whatiiif_Cloudflare_Worker.js` and `../sharing/public/iiif-core.js`). See "Duplicated logic" in `../CLAUDE.md` for the authoritative copy list. A parsing fix in one belongs in all of them. Extension-specific divergences are marked `EXTENSION NOTE:` in detect.js: chiefly, proxied fetches become direct fetches, and manifest URLs are always returned raw, never `/manifest-proxy`-wrapped.
