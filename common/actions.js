...
async function resolveAsyncBest(best, pageUrl) {
  // Match on the URL too, not just the name: several institutions have more
  // than one handler under a single display name (BHL /page/ vs /item/), and
  // picking the first by name alone routes a URL to a handler that can't
  // parse it — which returns null and looks like a resolution failure.
  const target = best.pageUrl || pageUrl || '';
  const named = PLATFORMS.filter(function (pl) { return pl.name === best.platform && pl.async; });
  const p = named.filter(function (pl) { return pl.pattern.test(target); })[0] || named[0];
  let resolved = null;
...
/* Page-aware content state with collection safety: fetch the manifest once.
   ...
   fetch fails. */
async function canvasTokenFor(manifestUrl, hint) {
  try {
    ...
    const index = resolveHintIndex(hint, cvs);
    if (index < 0) return null;
    const cid = canvasIdOf(cvs[index]);
    ...
    if (rs.ok) {
      ...
      if (looksLikeManifest(mj)) {
        ...
        if (vols.length) {
          ...
          if (hint) {
            const cs = await canvasTokenFor(volUrl, hint);
            if (cs) return cs;
          }
        ...
        if (hint && !hint.manifestIndex) {
          ...
          const idx = resolveHintIndex(hint, canvases);
          const cid = idx >= 0 ? canvasIdOf(canvases[idx]) : null;
          if (cid) return { token: buildCanvasContentState(manifestUrl, cid), url: manifestUrl, page: idx + 1, complete: true };
        }
...
async function highlightConfigFor(manifestUrl, hint) {
  ...
  const resolved = useHint ? resolveHintIndex(hint, canvases) : -1;
  const idx = resolved >= 0 ? resolved : 0;
  ...
  /* pages[] carries every canvas the overlay might switch to, resolved here
     because the overlay has no manifest access of its own (and shouldn't
     fetch from the host page). It's the same per-canvas shape as the
     top-level fields, so jumping pages is a swap, not a re-fetch. Cost is
     memory only — no extra requests — and manifests are already in hand. */
  const pages = canvases.map(function (c) {
    const d = getCanvasDimensions(c);
    return {
      canvasId: canvasIdOf(c),
      imageUrls: canvasImageUrls(c),
      cW: d.cW,
      cH: d.cH,
