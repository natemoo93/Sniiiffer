...
/* Actions a click can still be bound to. 'copy-content-state' was removed:
   its token was only page-accurate when the viewer exposed a page number in
   its URL and silently degraded to a whole-manifest token otherwise — and on
   this path the only feedback is a badge flash, so the user couldn't tell
   which they got. The highlight overlay's page field covers the need. */
const CLICK_ACTIONS = ['popup', 'highlight', 'copy-url', 'open-whatiiif'];

/* Existing users may have the removed value in synced storage. Rewrite it
   rather than just ignoring it: storage.sync persists across machines, and a
   stale value would leave the options page showing the default (its radio
   lookup falls back to the first entry) while the click did something else. */
function getClickAction() {
  return new Promise(function (resolve) {
    chrome.storage.sync.get({ clickAction: CLICK_DEFAULT }, function (d) {
      const stored = (d && d.clickAction) || CLICK_DEFAULT;
      if (CLICK_ACTIONS.indexOf(stored) === -1) {
        chrome.storage.sync.set({ clickAction: CLICK_DEFAULT });
        resolve(CLICK_DEFAULT);
        return;
      }
      resolve(stored);
    });
...
async function handleClick(tab) {
  ...
  try {
    if (action === 'copy-url') {
      ...
      // detect.js ships with highlight.js — see the popup's equivalent call:
      // an injected file has its own top-level scope, so it can't see the
      // content script's copy.
      await chrome.scripting.executeScript({
        target: { tabId: tab.id },
...
chrome.runtime.onMessage.addListener(function (msg, sender, sendResponse) {
  ...
  /* Manifest-backed work runs HERE, never in the popup: a popup document is
     an ordinary chrome-extension:// origin whose fetch() is subject to CORS,
     and host_permissions does not exempt it. Institutions that don't serve
     ACAO * (Yale, most CONTENTdm) failed there — silently, since both
     helpers fall back or return null. The worker's fetch carries the
     extension's host permissions and is not CORS-checked. */
  if (msg.type === 'sniiiffer:contentState') {
    contentStateFor(msg.url, msg.hint || null).then(sendResponse, function () {
      sendResponse({ token: buildManifestContentState(msg.url), url: msg.url, page: null, complete: false });
    });
    return true;
  }
  if (msg.type === 'sniiiffer:resolveAsync') {
    resolveAsyncBest(msg.best, msg.pageUrl || '').then(sendResponse, function () {
      sendResponse({ platform: msg.best && msg.best.platform, source: 'platform', confidence: 'candidate', resolveFailed: true });
    });
    return true;
  }
  if (msg.type === 'sniiiffer:highlightConfig') {
    highlightConfigFor(msg.url, msg.hint || null).then(function (cfg) {
      sendResponse({ cfg: cfg || null });
    }, function () { sendResponse({ cfg: null }); });
    return true;
  }
});
